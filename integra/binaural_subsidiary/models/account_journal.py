...
class AccountJournal(models.Model):
    ...
    def _get_distinc_subsidiaries_where_journal_was_used_by_model(self, model_name, subsidiary_field_name, journal_field_name):
        self.ensure_one()
    ...
    def _get_distinc_subsidiaries_where_journal_was_used(self):
        ...
        # There is no need for account_payment, because when you create a payment record, this creates an account_move and the journal cannot be changed and when you change the subsidiary the value of the payment field of the related account_move branch changes to the same
        account_move_count = self._get_distinc_subsidiaries_where_journal_was_used_by_model("account_move", "account_analytic_id", "journal_id")
        purchase_order_count = self._get_distinc_subsidiaries_where_journal_was_used_by_model("purchase_order", "account_analytic_id", "journal_invoice_id")
        
        subsidiaries = [ dict_subsidiary.get('account_analytic_id', None) for dict_subsidiary in account_move_count + purchase_order_count]
    ...
    def _constraint_change_subsidiary_id(self):
        for record in self:
            distinc_subsidiaries_where_journal_was_used = record._get_distinc_subsidiaries_where_journal_was_used()
            ...
            if count_distinc_subsidiary == 1 and record.subsidiary_id.id in distinc_subsidiaries_where_journal_was_used:
                continue

            raise ValidationError(
                _(
                    "The journal has been used in payments, account movements or purchase orders"
                )
            )
