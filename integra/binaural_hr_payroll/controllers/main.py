...
class HrPayroll(HrPayroll):
    ...
    def get_payroll_report_print(self, list_ids="", **post):
        ...
        for report, slips in payslip_reports.items():
            for payslip in slips:
                ...
                reader = PdfFileReader(
                    io.BytesIO(pdf_content), strict=False, overwriteWarnings=False
                )
        ...
        if len(payslips) == 1:
            report_name = "%s - %s - %s - %s" % (
                "Nómina",
                payslips.struct_id.name,
                payslips.employee_id.name,
                format_date(request.env, payslips.date_from, date_format="MMMM y", lang_code=lang),
            )
        else:
            report_name = "Nómina"
