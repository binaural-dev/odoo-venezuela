...
class JournalReportCustomHandler(models.AbstractModel):
    ...
    def _get_move_line_additional_col(
        self, options, current_balance, values, is_unreconciled_payment
    ):
        ...
        if values["journal_type"] in ["sale", "purchase"]:
            ...
            if values["taxes"]:
                ...
                tax_val = _(
                    "B: %s",
                    report.format_value(
                        values["tax_base_amount"], blank_if_zero=False, figure_type="monetary"
                    ),
                )
                ...
                tax_val = _(
                    "B: %s",
                    report.format_value(
                        values["debit"], blank_if_zero=False, figure_type="monetary"
                    ),
                )
            ...
            if (
                values["account_type"] not in ("liability_credit_card", "asset_cash")
                and current_balance
            ):
                additional_col = [
                    {
            ...
            if (
                self.user_has_groups("base.group_multi_currency")
                and values["move_line_currency"] != values["company_currency"]
            ):
                amount = (
                    -values["amount_currency"]
                    if not is_unreconciled_payment
                    else values["amount_currency"]
                )
                additional_col[-1] = {
