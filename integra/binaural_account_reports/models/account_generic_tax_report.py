            ...
            for row in self._cr.dictfetchall():
                ...
                if is_tax_line:
                    if (
                        ...
                        pass
                    elif row["tax_type_tax_use"] == "none" and child_to_group_of_taxes.get(
                        row["tax_id"]
                    ):
                        ...
                        if group_tax_id not in group_of_taxes_with_extra_base_amount:
                            ...
                            results[group_tax_info["type_tax_use"]]["children"][group_tax_id][
                                "base_amount"
                            ][column_group_key] += row["base_amount"]
                            group_of_taxes_with_extra_base_amount.add(group_tax_id)
                    else:
                        tax_type_tax_use = (
                            row["src_group_tax_type_tax_use"] or row["src_tax_type_tax_use"]
                        )
                        results[tax_type_tax_use]["children"][row["tax_id"]]["base_amount"][
                            column_group_key
                        ] += row["base_amount"]
                else:
                    if (
                        ...
                        for child_tax_id in group_info["child_tax_ids"]:
                            results[group_info["type_tax_use"]]["children"][child_tax_id][
                                "base_amount"
                            ][column_group_key] += row["base_amount"]
                    else:
