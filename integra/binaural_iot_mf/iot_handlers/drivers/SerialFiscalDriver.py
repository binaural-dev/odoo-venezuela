...
class PrinterController(http.Controller):
    response_error = {
        "valid": False,
        "message": """
                No se ha podido conectar con la maquina fiscal,
                por favor verifique si encendida o conectada.
                """,
    }
    ...
    def check_connection(self, request):
        fiscal_printer = next(
            (
                ...
                if iot_devices[d].device_type == "fiscal_data_module"
                and iot_devices[d].device_connection == "serial"
                and iot_devices[d].identifier == request["identifier"]
            ),
            None,
        )
        return fiscal_printer

    @http.route("/hw_proxy/status", type="json", auth="none", cors="*")
    def status_error(self, data):
        request = json.loads(data)
        fiscal_printer = self.check_connection(request)
        if fiscal_printer:
            return iot_devices[fiscal_printer].ReadFpStatus(True)
        return self.response_error

    @http.route("/hw_proxy/debug/<identifier>/<cmd>", type="http", auth="none", cors="*")
    def debug(self, identifier, cmd):
        pre = "/dev/serial/by-path/"
        fiscal_printer = next(
            (
                d
                for d in iot_devices
                if iot_devices[d].device_type == "fiscal_data_module"
                and iot_devices[d].device_connection == "serial"
                and iot_devices[d].identifier == pre + identifier
            ),
            None,
        )
        if fiscal_printer:
    ...
    @http.route("/hw_proxy/programacion", type="json", auth="none", cors="*")
    def test_logging(self, data):
        request = json.loads(data)
        fiscal_printer = self.check_connection(request)
        if fiscal_printer:
            return iot_devices[fiscal_printer].programacion(True)
        return self.response_error

    @http.route("/hw_proxy/reprint", type="json", auth="none", cors="*")
    def reprint(self, data):
        request = json.loads(data)
        fiscal_printer = self.check_connection(request)
        if fiscal_printer:
            return iot_devices[fiscal_printer].reprint(request)
        return self.response_error

    @http.route("/hw_proxy/out_invoice", type="json", auth="none", cors="*")
    def out_invoice_action(self, data):
        request = json.loads(data)
        fiscal_printer = self.check_connection(request)
        if fiscal_printer:
            return iot_devices[fiscal_printer].print_out_invoice(request)
        return self.response_error

    @http.route("/hw_proxy/out_refund", type="json", auth="none", cors="*")
    def out_refund_action(self, data):
        request = json.loads(data)
        fiscal_printer = self.check_connection(request)
        if fiscal_printer:
            return iot_devices[fiscal_printer].print_out_refund(request)
        return self.response_error

    @http.route("/hw_proxy/machine", type="json", auth="none", cors="*")
    def machine_info(self, data):
        request = json.loads(data)
        fiscal_printer = self.check_connection(request)
        if fiscal_printer:
            return iot_devices[fiscal_printer].get_last_invoice_number(True)
        return self.response_error

    @http.route("/hw_proxy/report_x", type="json", auth="none", cors="*")
    def report_x(self, data):
        request = json.loads(data)
        fiscal_printer = self.check_connection(request)
        if fiscal_printer:
            return iot_devices[fiscal_printer].PrintXReport(True)
        return self.response_error

    @http.route("/hw_proxy/report_z", type="json", auth="none", cors="*")
    def report_z(self, data):
        request = json.loads(data)
        fiscal_printer = self.check_connection(request)
        if fiscal_printer:
            return iot_devices[fiscal_printer].PrintZReport()
        return self.response_error

    @http.route("/hw_proxy/S1", type="json", auth="none", cors="*")
    def S1(self, data):
        request = json.loads(data)
        fiscal_printer = self.check_connection(request)
        if fiscal_printer:
            return iot_devices[fiscal_printer].GetS1PrinterData(True)
        return self.response_error
...
class SerialFiscalDriver(SerialDriver):
    ...
    def _print_out_refund(self, invoice):
        ...
        try:
            ...
            if status["data"]["status"]["code"] not in ["1", "4"]:
                ...
                raise Exception("La maquina no esta lista para realizar una nota de credito")
    ...
    def _print_out_invoice(self, invoice):
        ...
        try:
            ...
            if status["data"]["status"]["code"] not in ["1", "4"]:
                _logger.warning(status["data"]["status"]["code"])
                raise Exception("La maquina no esta lista para realizar una factura")
    ...
    def _GetStatusError(self, st, er):
        ...
        status_codes = {
            ...
            "0x62": {"msg": "En modo fiscal y en emisi�n de documentos no fiscales", "code": "6"},
            "0x61": {"msg": "En modo fiscal y en emisi�n de documentos fiscales", "code": "5"},
            "0x60": {"msg": "En modo fiscal y en espera", "code": "4"},
            "0x42": {"msg": "En modo prueba y en emisi�n de documentos no fiscales", "code": "3"},
            "0x41": {"msg": "En modo prueba y en emisi�n de documentos fiscales", "code": "2"},
