...
class TestIvaRetentionUsd(AccountRetentionTestCommon):
    ...
    def test_account_retention_line_compute_vef_base(self):
        ...
        self.assertEqual(float_compare(retention_lines_data[0]["foreign_retention_amount"], 24, 2), 0)
        self.assertEqual(float_compare(retention_lines_data[0]["retention_amount"], 1.2, 2), 0)
        self.assertEqual(retention_lines_data[1]["aliquot"], 8)
        self.assertEqual(float_compare(retention_lines_data[1]["foreign_retention_amount"], 6, 2), 0)
        self.assertEqual(float_compare(retention_lines_data[1]["retention_amount"], 0.3, 2), 0)
        self.assertEqual(retention_lines_data[2]["aliquot"], 31)
        self.assertEqual(float_compare(retention_lines_data[2]["foreign_retention_amount"], 23.25, 2), 0)
        self.assertEqual(float_compare(retention_lines_data[2]["retention_amount"], 1.1625, 4), 0)
