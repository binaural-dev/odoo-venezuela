...
class RetentionIslrReport(models.TransientModel):
    ...
    date_end = fields.Date('Date end', default=date.today().replace(day=1) + relativedelta(months=1, days=-1))
    ...
    def print_report(self):
        ...
        if not table.empty and name:
            if report == 'islr':
                filecontent = report_obj._excel_file_retention_islr(table, name, start, end, current_company)
        if not filecontent:
    ...
    def _retention_islr_excel(self, current_company = False):
        ...
        for i in docs:
            for il in i.retention_line_ids:
                ...
                dict['Monto Operación'] = il.foreign_invoice_amount if foreign_currency_id == 3 else il.invoice_amount
