...
test("no_upload=True oculta el botón Subir en la vista lista de account.move", async () => {
    ...
    await openListView("account.move", {
        context: { default_move_type: "out_refund", no_upload: true },
        arch: `<list js_class="account_tree"><field name="name"/></list>`,
    });
    expect(document.querySelector(".o_button_upload_bill")).toBe(null);
});

test("sin no_upload, el botón Subir se muestra normalmente en la vista lista", async () => {
    await startServer();
    await start();
    await openListView("account.move", {
...
test("no_upload=True oculta el botón Subir en la vista kanban de account.move", async () => {
    await startServer();
    await start();
    await openKanbanView("account.move", {
        context: { default_move_type: "in_refund", no_upload: true },
...
test("sin no_upload, el botón Subir se muestra normalmente en la vista kanban", async () => {
    await startServer();
    await start();
    await openKanbanView("account.move", {
