...
class AccountJournal(models.Model):
    ...
    def _fill_payment_account_id_from_default(self):
        ...
        This can't be its own dedicated compute depending on
        inbound/outbound_payment_method_line_ids.payment_account_id: those
        collections are themselves generated by the native
        _compute_inbound/outbound_payment_method_line_ids computes, so a
        separate compute would conflict/be redundant with them. Instead this
        runs as a post-step inside those same computes.

        It's called from BOTH overrides (not just inbound) because Odoo
        doesn't guarantee outbound's compute runs after inbound's on every
        create/write; if outbound generated its lines after inbound already
        ran this fill, they'd be left without a payment_account_id. Calling it
        from both is redundant in the common case (it re-scans the union of
        both collections each time) but is idempotent and safe.
        """
        for journal in self:
