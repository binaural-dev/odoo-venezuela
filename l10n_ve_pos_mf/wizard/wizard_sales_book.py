...
class SalesBookPOS(models.TransientModel):
    ...
    date_from = fields.Date(
        required=True, default=datetime.today().replace(day=1))
    date_to = fields.Date(
        required=True,
        default=datetime.today().replace(day=1) + relativedelta(months=1, days=-1),
    )
    ...
    def get_lines_account_move(self):
        ...
        vef_base = self.company_id.currency_id.id == self.env.ref(
            "base.VEF").id
        ...
        for report in agrouped_by_report_z.items():
            ...
            for index, move in enumerate(report[1]):
                ...
                total_with_tax += (
                    move.amount_total if vef_base else move.foreign_amount_total
                )
                ...
                if (
                    ...
                    date_retention = move.retention_iva_line_ids[
                        0
                    ].retention_id.date_accounting
                    retention_receipt = move.retention_iva_line_ids[
                        0
                    ].retention_id.number
                    iva_retention = sum(
                        move.retention_iva_line_ids.mapped("retention_amount")
                    )
                    if not vef_base:
                        iva_retention = sum(
                            move.retention_iva_line_ids.mapped(
                ...
                lines_to_sales_book = {
                    ...
                    "type": move.sales_book_type,
                    "invoice_affected": (
                        move.reversed_entry_id.mf_invoice_number
                        if move.reversed_entry_id
                        else ""
                    ),
                    "date_retention_receipt": (
                        self._format_date(
                            date_retention) if date_retention else ""
                    ),
        ...
        for move in not_moves:
            ...
            lines_to_sales_book = {
                ...
                "date_retention_receipt": (
                    self._format_date(date_retention) if date_retention else ""
                ),
                "retention_receipt": retention_receipt or "",
                "total_with_tax": 0,
                "total_without_tax": 0,
    ...
    def sales_book_abstract(self):
        return [
            ...
            {
                "name": "Total de las Ventas Internas Afectadas en alícuota Reducida",
                "field": "",
            },
            {
    ...
    def generate_sales_book(self, date_from, date_to):
        ...
        for record in self:
            ...
            workbook = xlsxwriter.Workbook(
                file, {"in_memory": True, "nan_inf_to_errors": True}
            )
            ...
            worksheet.merge_range(
                ...
                workbook.add_format(
                    {"bold": True, "center_across": True, "font_size": 18}
                ),
            )
            ...
            for index, field in enumerate(record.sales_book_fields()):
                ...
                worksheet.merge_range(6, index, 7, index,
                                      field["name"], cell_bold)
                for index_line, line in enumerate(lines):
                    if field["field"] == "index":
                        ...
                        if field.get("number"):
                            worksheet.write(
                                8 +
                                index_line, index, line[field["field"]
                                                        ], cell_number
                            )
                        else:
                            worksheet.write(
                                8 + index_line, index, line[field["field"]])
                if field.get("number"):
                    row_start = xlsxwriter.utility.xl_rowcol_to_cell(8, index)
                    row_end = xlsxwriter.utility.xl_rowcol_to_cell(
                        7 + len(lines), index
                    )
                    ...
                    if field["field"] == "total_without_tax":
                        worksheet.write_formula(
                            13 + len(lines), 4, result_total, cell_number
                        )

                    if field["field"] == "tax_base_16":
                        worksheet.write_formula(
                            15 + len(lines), 4, result_total, cell_number
                        )

                    if field["field"] == "tax_amount_16":
                        worksheet.write_formula(
                            15 + len(lines), 5, result_total, cell_number
                        )

                    if field["field"] == "tax_base_8":
                        worksheet.write_formula(
                            16 + len(lines), 4, result_total, cell_number
                        )
                    ...
                    if field["field"] == "tax_base_31":
                        worksheet.write_formula(
                            17 + len(lines), 4, result_total, cell_number
                        )

                    if field["field"] == "tax_amount_31":
                        worksheet.write_formula(
                            17 + len(lines), 5, result_total, cell_number
                        )
                    ...
                    worksheet.write_formula(
                        ...
                        result_total,
                        workbook.add_format(
                            {"num_format": "#,##0.00", "top": True, "bold": True}
                        ),
                    )
