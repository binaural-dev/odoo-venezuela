...
from odoo.addons.l10n_ve_invoice_digital.services.tfhka_document_service import (
    VES_CURRENCY_NAMES,
)
    ...
    def _prepare_identification(self, picking, document_type, document_number):
        for record in picking:
            ...
            return {
                ...
                "sucursal": "",
                "tipoDeVenta": "Interna",
                "moneda": self._get_currency_code(record),
                "transaccionId": "",
                "urlPdf": ""
            }

    def _get_currency_code(self, picking):
        """Código TFHKA de la moneda del documento.

        En 17.0 estaba fijado a "VEF" literal, ignorando ``code_tfhka`` y la
        moneda real de la compañía.
        """
        company = picking.company_id
        if company.currency_id.name in VES_CURRENCY_NAMES:
            return company.currency_id.code_tfhka
        return company.foreign_currency_id.code_tfhka

    def _get_payment_type(self, picking):
        for record in picking.sale_id:
            # nb_days sobre un recordset de varias líneas lanza excepción.
            if any(term.nb_days > 0 for term in record.payment_term_id.line_ids):
                return "Crédito"
            else:
                return "Inmediato"
        ...
        for record in picking:
            if record.sale_id and record.transfer_reason_id.code == "sale":
                for move_line in record.move_ids:
                    ...
                    taxes = sale_line.tax_ids.filtered(lambda t: t.amount)
                    tax_rate = taxes[0].amount if taxes else 0.0

                    # 17.0 comparaba solo con "VEF": una compañía en "VES" caía
                    # en la rama de divisa y reportaba foreign_price por error.
                    if record.sale_id.currency_id.name in VES_CURRENCY_NAMES:
                    ...
                    # tax_ids puede traer varios impuestos: .amount sobre el
                    # recordset completo revienta. Se usa la alícuota ya resuelta.
                    vat = round(item_price * tax_rate / 100, 2)
