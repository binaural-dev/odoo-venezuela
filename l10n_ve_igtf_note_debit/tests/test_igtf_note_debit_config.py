...
class TestIgtfNoteDebitConfig(TransactionCase):
    ...
    def test_valid_journal_ids_when_company_currency_is_vef_allows_no_currency(self):
        ...
        # Compañía aislada (sin plan de cuentas/asientos ya posteados): la
        # compañía principal ya tiene asientos de apertura del plan de
        # cuentas y Odoo bloquea el cambio de moneda cuando ya existen
        # apuntes contables.
        company = self.env["res.company"].create({
            "name": "Compañía VEF (config test)",
            "currency_id": self.currency_vef.id,
        })
        ...
        self.assertIn(
            implicit_vef_journal.id, company.igtf_note_debit_valid_journal_ids
        )
    ...
    def test_valid_journal_ids_when_company_currency_is_not_vef_requires_explicit_vef(self):
        ...
        # Compañía aislada por la misma razón que el test anterior.
        company = self.env["res.company"].create({
            "name": "Compañía USD (config test)",
            "currency_id": self.currency_usd.id,
        })
        cash_account = self.env["account.account"].create({
        ...
        valid_ids = company.igtf_note_debit_valid_journal_ids
...
class TestIgtfNoteDebitResConfigSettings(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.exempt_purchase_tax = cls.env["account.tax"].create({
            "name": "Exento Compra (settings test)", "amount": 0.0,
            "amount_type": "percent", "type_tax_use": "purchase",
            "company_id": cls.company.id,
        })
        cls.igtf_product = cls.env["product.product"].create({
