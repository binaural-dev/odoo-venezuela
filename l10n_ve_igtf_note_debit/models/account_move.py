...
class AccountMove(models.Model):
    ...
    l10n_ve_igtf_note_debit_origin = fields.Boolean(
        ...
        help="True if this Debit Note was automatically generated by the "
             "IGTF Debit Note flow (l10n_ve_igtf_note_debit).",
    )
    ...
    def prepare_igtf_payment_debit_note(self, igtf_amount_company_curr, invoice, payment):
        ...
        if not product:
            raise UserError(_(
                ...
                "Accounting > IGTF before using the 'Automatic Fiscal Debit "
                "Note' mode."
            ))
        ...
        if not debit_note_id:
            raise UserError(_("The Debit Note wizard did not return a valid ID."))
    ...
    def _settle_igtf_debit_note_with_vef_payment(self, debit_note, source_payment):
        ...
        if not journal:
            raise UserError(_(
                "Configure the VEF journal for IGTF collection in Settings "
                "> Accounting > IGTF (or create a bank/cash journal in "
                "Bolivares without marking it as IGTF)."
            ))
        ...
        vef_payment = self.env["account.payment"].create({
            ...
            "memo": _("IGTF on %s") % debit_note.name,
        })
