...
class PosSession(models.Model):
    ...
    def _get_pos_ui_product_product(self, params):
        if self.company_id.pos_show_free_qty_on_warehouse:
            self = self.with_context(warehouse=self.config_id.warehouse_id.id)
                ...
                if not order_payment.payment_method_id.apply_one_cross_move:
                    if (
                        ...
                        if order_payment.amount < 0:
                            line_vals = session._line_vals_move_cross_outgoing(
                                order_payment
                            )
                        else:
                            line_vals = session._line_vals_move_cross_incoming(
                                order_payment
                            )
    ...
    def set_foreign_amount_in_line(self, line, foreign_amount, amount=0.0):
        ...
        if (
            abs(line.debit) > 0
            and float_compare(
                line.debit, abs(amount), precision_rounding=self.currency_id.rounding
            )
            == 0
        ):
            line.not_foreign_recalculate = True
