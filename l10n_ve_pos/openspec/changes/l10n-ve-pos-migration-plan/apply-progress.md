...
- [x] **HD.5** — ⚠️ **POR REVISAR + PRUEBAS UNITARIAS PENDIENTES** (crítico antes de mergear). Centralización completa de la conversión foránea:
  * **API canónica única**: `pos.config._convert(from_amount, from_currency, to_currency, round=True)` en Python (nuevo helper en `models/pos_config.py`) y `PosOrder._convert(amount, fromCurrency, toCurrency, doRound)` en JS (nuevo en `static/src/overrides/models/pos_order.js`). Ambos con el mismo contrato: multiplican por la tasa cruda (todos los dígitos de `foreign_rate`, sin redondeo temprano) y redondean solo el resultado con `to_currency.round()`. Documentado como MIRROR CONTRACT en ambos archivos.
  * **Semántica de tasa aclarada**: `foreign_rate` es SIEMPRE el multiplicador `main → foreign` (según `l10n_ve_rate.compute_rate`); `foreign_inverse_rate` NO se usa para aritmética (queda solo para reporting en `account.move.line`). Docblock extenso en `_get_pos_conversion_rate` explica los dos setups (main=VEF/foreign=USD y main=USD/foreign=VEF).
  ...
  * **Pruebas unitarias requeridas antes de mergear** (crítico, cubren cálculos sensibles):
    1. `pos.config._convert`: main=VEF/foreign=USD (0.05 = 35.67 * foreign_rate); main=USD/foreign=VEF (24074 = 35.67 * foreign_rate); currency igual (identidad); currency desconocida (retorna 0.0); precisión con todos los dígitos de la tasa; round=False no redondea.
    2. `pos.config._get_pos_conversion_rate`: verifica que `foreign_rate` es SIEMPRE el multiplicador main→foreign en ambos setups.
