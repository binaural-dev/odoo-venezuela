...
class WizardAccountingReportsBinauralInvoice(models.TransientModel):
    ...
    def parse_sale_book_data(self):
        ...
        for date_moves in agrouped_by_date.items():
            ...
            for move in date_moves[1]:
                if not move.mf_serial:
                    key = f"free_{move.id}"
                else:
            ...
            for key, moves_group in agrouped_by_report_z.items():
                ...
                sorted_moves = moves_group.sorted(lambda m: int(m.mf_invoice_number or 0))

                for index, move in enumerate(sorted_moves):
                    next_move = move
                    is_last_move = False
                    if (index + 1) < len(sorted_moves):
                        next_move = sorted_moves[index + 1]
                    else:
                        is_last_move = True
