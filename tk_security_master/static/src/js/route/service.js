...
function urlToState(urlObj) {
    ...
    if (pathname === "/web") {
        ...
        const url = browser.location.origin + routerService.stateToUrl(state);
        urlObj.href = url;
    }
...
function startRouter() {
    ...
    state = routerService.urlToState(url);
    // ** url-retrocompatibility **
...
browser.addEventListener("popstate", (ev) => {
    ...
    state = ev.state?.nextState || routerService.urlToState(new URL(browser.location));
    ...
    if (!ev.state?.skipRouteChange && !routerService.skipLoad) {
        routerBus.trigger("ROUTE_CHANGE");
    }
    routerService.skipLoad = false;
});
...
function makeDebouncedPush(mode) {
    function doPush() {
        ...
        const url = browser.location.origin + routerService.stateToUrl(nextState);
        ...
        jsonrpc("/user/read/operation", {'url_hash': state, 'browser_url': browser.location.href });
    }
    /**
     * @param {object} state
