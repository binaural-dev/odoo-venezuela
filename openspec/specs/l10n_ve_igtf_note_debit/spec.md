# l10n_ve_igtf_note_debit

## Purpose

Agrega un flujo alterno y opcional de percepción de IGTF sobre el que ya
provee `l10n_ve_igtf`: en vez de contabilizar el IGTF como una línea dentro
del mismo asiento de pago/anticipo (flujo histórico "inline"), genera un
documento fiscal independiente -- una **Nota de Débito** (vía
`account_debit_note`) -- vinculada a la factura de origen, conforme a las
Providencias SENIAT 0071/0102. Es 100% opt-in por compañía
(`igtf_note_debit_mode`): mientras el modo esté en `inline`, este módulo
delega enteramente en `super()` sin ningún cambio de comportamiento.

Extiende `account.move` (`compute_bi_igtf`, `remove_igtf_from_account_move`,
`js_assign_outstanding_line`, `_create_advance_payment_move`,
`prepare_igtf_payment_debit_note`, `settle_igtf_debit_note`,
`create_note_credit_igtf`), `account.payment`
(`_create_igtf_moves_in_payments`), `account.payment.register`
(`_compute_amount`, `_onchange_amount`, `_compute_payment_difference`,
`_create_payments`), `res.company` y `res.config.settings` (configuración del
modo, producto de percepción, diario VEF, checkbox por defecto). Depende de
`l10n_ve_igtf` (motor de detección de IGTF, campo `is_igtf` del diario,
`calculate_igtf_for_payment`) y `account_debit_note` (wizard `account.debit.note`
que emite la ND en Forma Libre).
