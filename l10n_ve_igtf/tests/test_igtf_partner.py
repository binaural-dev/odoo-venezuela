...
from .new_igtf_common_partner import IGTFTestCommon 
...
class TestIGTFNEW(IGTFTestCommon): 
    ...
    def test02_payment_from_invoice_with_igtf_journal(self):
        ...
        expected_lines_2 = [
           ...
           
            {
                'account': self.acc_receivable,
                'debit': 0.0,
                'credit': cxc_credit_amount_2,   
            },
             {
                'account': self.acc_igtf_cli,  
                'debit': 0.0,
                'credit': expected_igtf_2,       
            },
        ]
    ...
    def test03_payment_from_invoice_with_igtf_journal(self):
        ...
        expected_lines_2 = [
            {'account': self.account_bank, 'debit': payment_amount_2, 'credit': 0.0},
            {'account': self.acc_receivable, 'debit': 0.0, 'credit': cxc_credit_amount_2},
            {'account': self.acc_igtf_cli, 'debit': 0.0, 'credit': expected_igtf_2},
        ]
        self._assert_move_lines_equal(payment_move_2, expected_lines_2)
        ...
        partial_reconcile = payment_2_receivable_line.matched_debit_ids.filtered(
    ...
    def test04_payment_from_invoice_with_igtf_journal_currency_usd(self):
        ...
        partial_reconcile = payment_1_receivable_line.matched_debit_ids.filtered(
            lambda p: p.debit_move_id == invoice_receivable_line
        )
        _logger.info(f"Partial Reconcile encontrado: {partial_reconcile.mapped(lambda p: (p.id, p.debit_move_id.id, p.credit_move_id.id))}")
        self.assertTrue(partial_reconcile, "No se halló la conciliación parcial (account.partial.reconcile) a eliminar.")
    ...
    def test05_payment_from_invoice_with_igtf_journal_mixed_currency(self):
        ...
        expected_lines = [
           ...
           
            {
                'account': self.acc_receivable,
                'debit': 0.0,
                'credit': cxc_credit_amount_1,   
            },
             {
                'account': self.acc_igtf_cli,  
                'debit': 0.0,
                'credit': expected_igtf_1,       
            },
        ]
        ...
        _logger.info("test03_payment_from_invoice_with_igtf_journal_mixed_currency (Flujo Desconciliación - VES) superado.")
    ...
    def test06_payment_from_invoice_with_overpayment_and_reconciliation(self):
        ...
        expected_lines_1 = [
            ...
            {
                'account': self.acc_receivable, # Línea que recibe el crédito aplicado y tendrá el sobrante
                'debit': 0.0,
                'credit': cxc_credit_amount_1,
            },
            {
    ...
    def test07_payment_from_invoice_with_overpayment_and_reconciliation(self):
        ...
        partial_reconcile_1 = outstanding_line.matched_debit_ids.filtered(
            lambda p: p.debit_move_id == invoice_1_payable_line
        )
