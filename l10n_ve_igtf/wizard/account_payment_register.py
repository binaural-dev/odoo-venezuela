...
class AccountPaymentRegisterIgtf(models.TransientModel):
    ...
    is_igtf = fields.Boolean(string="IGTF", help="IGTF")
    ...
    igtf_amount = fields.Float(string="IGTF Amount", help="IGTF Amount")
            ...
            if wizard.is_igtf:
                ...
                for invoice in move_ids:
                    igtf_for_invoice = wizard.calculate_igtf_for_payment(
                        invoice, base_amount, wizard.igtf_percentage
                    )
        ...
        for wizard in self:
            if wizard.can_edit_wizard and wizard.payment_date and wizard.is_igtf:
                ...
                if (
                    wizard.is_igtf
                    and float_compare(
                        wizard.igtf_to_show, 0.0, precision_rounding=precision
                    )
                    > 0.0
                ):
        ...
        for rec in self:
            ...
            for move_id in move_ids:
                ...
                residual = (
                    ...
                    if move_id.company_currency_id != self.env.ref("base.VEF")
                    else move_id.amount_residual / rec.foreign_inverse_rate
                )

                if rec.amount <= residual + residual * (rec.igtf_percentage / 100):
                    amount_without_difference = amount_without_difference + (
                        rec.amount - rec.igtf_to_show
                    )
        ...
        for payment in self:
            ...
            for rec in move_ids:
                if payment.is_igtf:
                    ...
                    amount = amount + payment.calculate_igtf_for_payment(
                        invoice, payment.amount, payment.igtf_percentage
                    )
            if payment.is_igtf:
    ...
    def calculate_igtf_for_payment(
        self, invoice, payment_amount, igtf_percentage=False
    ):
        self.ensure_one()
        ...
        due_amount = (
            float(self.source_amount)
            if invoice.company_currency_id != self.env.ref("base.VEF")
            else float(self.source_amount) / self.foreign_inverse_rate
        )
        ...
        if (
            not float_is_zero(igtf, precision_rounding=precision)
            and igtf_top == invoice_residual
        ):
        ...
        if igtf > residual_igtf and not float_is_zero(
            residual_igtf, precision_rounding=precision
        ):

            igtf = residual_igtf

        if (
            float_compare(igtf_top, 0.0, precision_rounding=precision) >= 0.0
    ...
    def _compute_is_igtf_journal(self):
        for record in self:
            if (
                ...
                and record.journal_id.currency_id == self.env.ref("base.USD")
            ):
        ...
        if create and create.journal_id.is_igtf:
            ...
            for invoice in move_ids:
                igtf_for_invoice = self.calculate_igtf_for_payment(
                    invoice, invoice.amount_residual, self.igtf_percentage
                )
                total_igtf_amount += igtf_for_invoice
            ...
            wizard_values["is_igtf_on_foreign_exchange"] = (
                create.is_igtf_on_foreign_exchange
            )
