...
class AccountMove(models.Model):
    ...
    igtf_percentage = fields.Float(
        string="IGTF Percentage", default=default_igtf_percentage
    )
            ...
            for line in self.env['account.move.line'].search(domain):
                ...
                if has_advance_line:
                    ...
                    payments_widget_vals['content'].append({
                        ...
                        #"foreign_id": line.foreign_currency_id.id,
    ...
    def _create_advance_payment_move(self, amount_residual, lines):
        ...
        cross = bool(lines.move_id.origin_payment_advanced_payment_id) 
        ...
        if is_customer:
            name_rp = "CUENTA POR COBRAR CLIENTE"
            name_adv = "ANTICIPO/CLIENTE"
            account_adv = self.env.company.advance_customer_account_id.id if cross else payment.destination_account_id.id
            igtf_account = self.env.company.customer_account_igtf_id.id
        else:
            name_rp = "CUENTA POR PAGAR PROVEEDOR"
            name_adv = "ANTICIPO/PROVEEDOR"
            account_adv = self.env.company.advance_supplier_account_id.id if cross else payment.destination_account_id.id
        ...
        if is_customer:
            ...
            igtf_amount = -igtf_amount if igtf_line == 'credit' else igtf_amount
        else:
        ...
        if float_compare(abs(igtf_amount), 0.0, precision_rounding=precision) > 0.0 and is_igtf_journal:
            line_vals.append(
                Command.create({
                "name": "IGTF",
        ...
        rate_values = Rate.compute_rate(self.currency_id.id, self.invoice_date)
        move_to_reconcile_with_payment_difference = self.env["account.move"].create({
            ...
            "foreign_inverse_rate" :rate_values.get("foreign_inverse_rate"),  #Se debe inyectar
    ...
    def js_assign_outstanding_line(self, line_id):
        ...
        if is_advance_payment:
            ...
            reconciled_result = self._reconcile_move_with_payment_difference(
    ...
    def cancel_advance_payment_transaction(self, origin_payment_id, partial_reconcile):
        ...
        if partial_reconcile.state == 'posted':
    ...
    def compute_bi_igtf(self):
        for rec in self:
            if rec.amount_total_signed > 0 or rec.payment_state in ['paid','in_payment']: 
                ...
                payment_moves = self.env['account.move'].with_company(rec.company_id)
        ...
        if igtf_line and receivable_payable_line:
            ...
            for line in payment_move.line_ids:
                ...
                if line.id == igtf_line.id:
                    ...
                    current_foreign_debit = line.foreign_debit
                    current_foreign_credit = line.foreign_credit
                    
                    if igtf_line_balance > 0: # IGTF DÉBIT
                        new_debit = current_debit + igtf_line_balance
                        new_credit = 0.0
                        new_foreign_debit = current_foreign_debit + abs(igtf_line.foreign_balance)
                        ...
                        new_foreign_credit = current_foreign_credit + abs(igtf_line.foreign_balance)

                    line_vals = {
                        'debit': new_debit,
                        'credit': new_credit,
                        'foreign_debit':new_foreign_debit,
                        'foreign_credit':new_foreign_credit,
